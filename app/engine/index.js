...
(function (root, factory) {
      ...
      for (var f in byField) {
        ...
        if (!angle) {
          /* 角度の表を持たない欄。**黙って落とさず名前で残す**(台帳 CROSS-3)。
             本数は `count`、読みの中身は `rows`(cycle-0160・台帳 CROSS-L2c で足した)。
             両方を持たせるのは冗長に見えるが、**片方へ寄せると下流の意味が黙って変わる**。

             **cycle-0163・台帳 CROSS-L2d で、本数の欄を `texts` から `count` へ改名した。**
             角度ありの行の `texts` は**読みの配列**なので、同じ返り物の中で同じ名前が
             二つの意味を持っていた。**その形は cycle-0160 の作業中に実際に踏んでいる**
             (点検の一時スクリプトで `el[0].texts || el[0].rows` と書き、数のほうが
             選ばれて `rows.map is not a function` で落ちた)。改名の前に読み手を走査で
             追ってある=**engine 側にこの欄を読む判定は1つも無い**
             (`angleRosterProblems` は欄名だけ・`angleTextCountProblems` は `rows` だけを
             読む)。読み手は tests/yomi.spec.js の4か所だけである。 */
          skipped.push({ key: sources[s].key, field: f,
                         count: byField[f].length, rows: byField[f] });
          continue;
        }
  ...
  /** **欄を保ったまま、読みの本数だけが落ちる壊れ方**を捕まえる(cycle-0158・台帳 CROSS-L2b)。
      ...
      **角度の表を持たない欄(exempt)も同じ点検に掛ける。**cycle-0158 のこの判定は
      `report.fields`(=角度の表を持つ欄)しか回しておらず、**角度の表を持たない欄**
      (算命学の中心の星・本元の気・天中殺の組/数秘の数の性質・数の重なり/
      西洋のエレメント・三区分/姓名判断の画の型)は、exempt として名前では
      集まっているのに**読みが何本あるかは誰も見ていなかった**=CROSS-L2b が塞いだ穴が
      そっくり同じ形でこちら側に残っていた(角度の表に載せないことと、読みが減っても
      よいことは別の話である)。

      **欄ごとの本数はここに書かない**(cycle-0163・台帳 CROSS-L2g)。cycle-0160 の
      この注記には欄ごとの読みの本数が手打ちで入っていたが、**増減しても何も落ちない
      数字**だった(#45 は数を実体から導ける形にせよと述べており、コメントは実体から
      導けない)。**数が実体から出るのは2か所**=検査 YOMI1-6 の表示文
      (`角度の表を持たない欄の顔ぶれ: …(◯本)`)と、報告書の `yomi_status`。
