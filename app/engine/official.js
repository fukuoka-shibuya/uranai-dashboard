...
(function (root, factory) {
  ...
  function sanmeiOfficial(b) {
    ...
    var items = [
      ...
      { label: '日の干支', value: KAN[dayKan] + SHI[dayShi],
        note: '生まれた日から出るこの二文字は、あなたの持ち味の土台にあたります。ほかの項目はここを起点に読み広げていくと見ます。' },
      ...
      { label: '月の干支', value: KAN[monthKan] + SHI[monthShi],
        note: '生まれた月から出るこの二文字は、日々の過ごし方ににじみやすいあなたの面を表します。ふだんの暮らしぶりに出やすいところと見ます。' },
    ...
    return {
      ...
      summary: '暦を正式にたどると、生まれた日を表す二文字は「' + KAN[dayKan] + SHI[dayShi] + '」、そこから導いた中心の星は「' +
               centerStar + '」と出ています。年・月・日それぞれの二文字を組み合わせて、暮らしの手触りを' +
               '六つの窓から眺めていきます。',
  ...
  function kyuseiOfficial(b) {
    ...
    return {
      ...
      items: [
        ...
        /* Issue #51:「盤」「定位」の語を画面から外し、その方角が読み手にとって
           何の目安になるかを述べる形へ書き替えた(語り口は「〜ではありません。」も可) */
  ...
  /* Issue #51 で「系統」の語を画面から外したため、この一文からも語だけを外す
     (打ち消しそのものは消さない) */
  var KEITOU_DISCLAIMER = 'この呼び名は経典に伝わる古い言い方で、人の善し悪しを表すものではありません。';
  ...
  function sukuyoOfficial(b) {
    ...
    return {
      ...
      summary: '生まれた日を月の満ち欠けの暦に直すと' + lunarDateText(lunar) + 'にあたり、' +
               ...
               keitou + 'の型に置かれる宿として、人との間にどう調子が流れるかを眺めていきます。',
      ...
      items: [
        ...
        { label: '旧暦の生まれ日', value: lunarDateText(lunar) +
          ...
          note: '人が昔使っていた月の満ち欠けの暦に、あなたの生まれた日を置き直した日付です。あなたの宿はこの日付から数えて決まります。' },
        { label: '宿の系統', value: keitou + 'の型',
  ...
  function overallOfficial(b) {
    ...
    return {
      ...
      view: '五つの見方の重なり',
      heading: '五つの見方を重ねて',
      /* Issue #51:書き出しの列挙から「三区分」「宿の系統」の語を外し、画面の値と
         同じ言い方(星座の型・宿の型)へそろえた。項目名(label)そのものは
         engine/index.js の ABOUT が項目名で引く作りのため動かしていない */
