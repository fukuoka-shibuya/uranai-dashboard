...
(function (root, factory) {
  ...
  /* 総合占いは個別占術の文の再掲・連結にしない。中核5占術の「正式計算が出した結果値
     ...
     ただし10行のうち次の2行は、その値そのものの説明文が画面に無いため、別の文言からの
     読み替え・当てはめである(cycle-0043 の監査 M1 を受けて正直に明示する。台帳 OC43-L2):
      ・九星の間合い(KYUSEI_MAAI):添字は月命星だが、根拠に引いた KYUSEI_NOTE は
        本命星の項目にしか出ない文言。月命星の note 自身は距離の取り方に触れていない
      ・数秘の間合い(SUUHI_BIRTH_MAAI):添字は誕生数だが、根拠の LIFEPATH_NOTE は
        ライフパスナンバーの項目にしか出ない文言
     残る8行(算命学2・九星の動き出し・数秘の動き出し・西洋2・宿曜2)は、
     引用した説明文が画面の note の実文言と一致する。
     宿曜の動き出しは cycle-0044 まで3行目の未根拠に数えていたが、cycle-0045 で
     宿の系統の文(後半=事の始め方)から写す形に取り直した。

     ただし取り直した7行のうち次の2つは、文から向きへの導出がまだ弱い
     (cycle-0045 の監査 M1・M2。台帳 OC45-M1/M2 として次サイクルで書き替える):
      ・剛柔:追記文は踏み出す速さが場面で移り変わることを述べるが、「何に合わせるか」を
        述べていない。動き出しの軸に「変える」型が無いため、可変性の行が消去法で
        「合わせる」へ落ちている
      ・毒害:追記文が「心が決まってからの運びは速く」と関門を含むため、猛悪の
        「腑に落ちてから力を出していく」(=確かめる)と同じ形の文でありながら
        逆の「先へ動く」へ写している。前半を読み飛ばして後半だけを採る形になっている
  ...
  /* 宿曜:宿の系統を動き出しに写す。添字は SHUKU_KEITOU_NAME の並び。
     ...
     毒害=決まってからの運びが速くためらいを抱えない→先へ動く/
     猛悪=己の納得が先に立ち腑に落ちてから力を出す→確かめる/
     剛柔=踏み出す速さが場面で入れ替わる→合わせる。
