...
(function (root, factory) {
  ...
  /* cycle-0078(#55・工程5=切替):ここにあった DAY_KANSHI_NOTE(生まれた日の二文字の
     ...
     ---- cycle-0161(台帳 SANMEI-5b):この欄立てのままで残すと決めた ----

     cycle-0078 のこの削除で、仮計算6欄と正式計算6欄の顔ぶれが大きく食い違った
     (実測:共通は中心の星・本元の気・天中殺の組の3欄。仮だけが気の配り方・
     人との間合い・動き出しの型、正だけが日/年/月の干支)。台帳 SANMEI-5b は
     「仮計算の算命学そのものを縮めるか、このままにするか」を決める行だった。
     **このままにする。**理由は3つで、どれも実測に基づく。

     (1) **正式へそろえる道は取れない。**正式だけが持つ3欄は六十干支ごとの読み
         (KANSHI_YOMI・180本)を要するが、仮計算はその表を持てない=CLAUDE.md の
         絶対条件「仮計算と正式計算はコード上分離」。official.js の表をこちらから
         引けば、仮計算へ戻すという安全側の作りそのものが壊れる。
     (2) **共通の3欄へ縮める道も取れない。**縮めると算命学(中核の中心・最も厚く
         読む占術)が仮計算では3欄になる。ご指示 #58 は「削除して欄が少なくなる
         場合は残った欄の読みを厚くして補う」と述べているが、厚い読みを持てないのが
         (1) の理由そのものなので、補いようが無い。
     (3) **仮だけの3欄は「読みを持たない欄」に当たらない。**ご指示 #58 の基準1が
         名指しするのは読みの無い欄で、この3欄はいずれも読みを持っている(実測で
         30〜44字。気の配り方32〜44・人との間合い32〜41・動き出しの型30〜38)。
         短いことは事実だが、それは仮計算の全体に等しく言えることで
         (実測:九星28〜41字・数秘28〜44字・宿曜37〜45字。西洋と姓名判断と宿曜の
         生まれの宿は note を空文字で置き、合流点の VALUE_NOTE から厚い読みを
         受け取るので長い)、算命学だけの問題ではない。**画面に出るのは正式計算
         だけなので読み手への実害は無い。**
         **この4つの数は 1930〜2020年の1092日を走らせた全走査の値である**
         (cycle-0161 の点検役 中5。**最初に書いた「27〜37字・九星33〜41字・
         数秘30〜44字・宿曜45字」は3日だけを走らせて測った値で、値の顔ぶれを
         出し切っていなかった**=cycle-0086 の教訓と同じ形。しかも「27字」は
         仮だけの3欄ではなく共通の「本元の気」の値で、欄を取り違えていた。
         **「実測」と書くときは、その走査が値の顔ぶれを出し切っているかまで見ること。**)

     残す以上、欄の数を語る文が欄の数と食い違わないことは要る=それが下の
     入れ札(cycle-0161・台帳 GUARD-3)である。あわせて、この区画の
     MAIN_STAR_NOTE(10本)と GOGYO_NOTE(5本)が official.js の同名の表と別の
     文であること(点検役 cycle-0081 の M3 が名指しした二重管理)も、上の (1) から
     出る当然の帰結として**そのままにする**=分離の決まりがある以上、同じ文を
     持たせる道は無い。 */
  ...
  function sanmei(b) {
    ...
    return {
      ...
      /* cycle-0161(台帳 GUARD-3):窓の数をここで書かず、入れ札(`{欄数}`)を置く。
         cycle-0159 で歯止め(読みが引けなければ欄を出さない)が合流点へ移ったため、
         **欄が落ちるのはこの関数を出たあと**になった。ここで items.length を読むと
         歯止めより手前の数になり、読みが1件抜けた回に欄は5つなのに導入文だけが
         「六つの窓」と言い続ける。数は欄が確定する場所=index.js の withGuide が埋める */
      summary: '中心の星は「' +
  ...
  /* cycle-0092(台帳 YOMI-7a):色と定位の対応表(KYUSEI_IRO / KYUSEI_HOUI)は
     official.js と同じ回にここからも消した。「星の色」「定位の方角」の2欄を
     ご指示 #58 の基準1により画面から落とし、引き手が無くなったため。
     **片方だけ消すと、仮計算へ戻したときにだけ読みの無い欄が現れる**ので、
     欄の削除は仮・正式の両方を同じ回で行う(OC51-1b・OC51-4 と同じ考え方) */
  ...
  function kyusei(b) {
    ...
    return {
      ...
      items: [
        ...
        /* cycle-0092(台帳 YOMI-7a):ここにあった「星の色」「定位の方角」の2欄も
           official.js と同じ回で落とした。cycle-0061 の書き分けの文は両実装で
           同一に保つ決まりだったので、片方だけ残せばその決まりが破れる */
      ]
    };
  }
    ...
    return {
      ...
      items: [
        ...
        { label: '生まれの宿', value: SHUKU[i],
          /* cycle-0089(台帳 YOMI-5b):読みは合流点の VALUE_NOTE.sukuyo 1か所へ移した。
             値の文字は仮計算でも正式計算でも同じ二十七宿の名なので、実装ごとに
             同じ文を持たない(CLAUDE.md の言葉づかいの条項) */
          note: '' },
        { label: '宿の系統', value: kei + 'の型', note: SHUKU_KEITOU_NOTE[kei] },
        /* 「巡りの位置」は cycle-0093(台帳 YOMI-7b)で仮計算からも落とした。
           ご指示 #58 の基準1=読みを持たない欄は表示しない。cycle-0064 以来この欄は
           書き分け(行き先の名指し)で、27通りとも同じ文だった。**仮・正式の片方だけ
           消すと、仮計算へ戻したときにだけ読みの無い欄が現れる**ので同じ回で両方
           消してある(cycle-0092 の九星と同じ扱い)。文は official.js の
           CYCLE_POS_NOTE と同一文で二重に持っていたが、そちらも同じ回で消えている */
  ...
  var KANA_STROKES = {
    ...
    'ヤ': 2, 'ユ': 2, 'ヨ': 3,
    'ラ': 2, 'リ': 2, 'ル': 2, 'レ': 1, 'ロ': 3,
    /* ヰ=4:正式化の工程5で外部2出典一致の4に決着し、工程6(切替の回)で
       正式計算側と同時に適用した(SEIMEI2-1 が両実装の表の1字1値の一致を
       要求するため片側だけでは変えられない)。適用前の値は3 */
    'ワ': 2, 'ヰ': 4, 'ヱ': 3, 'ヲ': 3, 'ン': 2,
    'ァ': 2, 'ィ': 2, 'ゥ': 3, 'ェ': 3, 'ォ': 3,
    'ャ': 2, 'ュ': 2, 'ョ': 3, 'ッ': 3,
    'ー': 1
  ...
  /* 画の型(総画を一桁に畳んだ1〜9)の読みは、cycle-0090(台帳 YOMI-6)で
     合流点の index.js の VALUE_NOTE.seimei 1か所へ移した。値の文字
     (一の型〜九の型)は畳んだ数の言い換えなので、正式計算へ切り替わっても
     同じ文字になる=実装ごとに文を持つと二重管理になる(宿曜「生まれの宿」と
     同じ扱い)。ここにあった1行×9本の表(KAKU_KATA_NOTE)は、その9本の
     1文目の見立て(ひと筆・二点…九升)として引き継がれている。
     note を空にすれば withGuide が VALUE_NOTE の読みだけを画面へ出す */
  ...
  function seimei(name) {
    ...
    return {
      ...
      items: [
        /* 「総画」の欄は cycle-0094(台帳 YOMI-7c)で画面から落とした。
           ご指示 #58 の基準1(読みを持たない欄は表示しない)に当たる欄で、
           値は名前の文字数に応じて際限なく散る(1文字でも1〜29)ため、
           値ごとの読みを書く道が無い(宿曜の「旧暦の生まれ日」と同じ形)。
           cycle-0064 では行き先(画の型)を名指しする書き分けで凌いでいたが、
           **書き分けは読みを持たせたことにはならない**ので削除へ倒した。
           **合計そのものは上の summary が述べ続ける**(「合わせて◯画になります」)
           ので、読み手が受け取る数は失われない。合計を一けたへ畳んだ「画の型」は
           9通りの読み(index.js の VALUE_NOTE.seimei)を持ったまま残る。
           落としても失うものが無いことは tests/engine.spec.js の YOMI7c-3 が見る */
        { label: '画の型', value: KANJI_KAZU[kata] + 'の型', note: '' },
        /* cycle-0091(台帳 YOMI-6b):値を「◯画」(charStrokes の値域どおり29通り)
           から偶奇の言い換え(奇数/偶数の2値)へそろえた。計算から出る材料が
           偶奇しか無いのに値だけ29通りに分かれていると、値の粒度が材料より細かく、
           根拠なく書き分ければ #45 違反・粗い本を貼れば完全一致の線に当たる
           (cycle-0090 の申し送り)。値の文字(奇数/偶数)は画数の偶奇の言い換えで
           正式化後も同じ文字になるので、読みは合流点の index.js の
           VALUE_NOTE.seimei 1か所にある(画の型と同じ扱い。note は空にする) */
        { label: '頭字の画', value: (head % 2 === 1 ? '奇数' : '偶数'), note: '' },
        { label: '結字の画', value: (tail % 2 === 1 ? '奇数' : '偶数'), note: '' }
      ]
    };
  }
