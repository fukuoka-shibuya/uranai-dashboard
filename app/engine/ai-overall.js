...
(function (root, factory) {
  ...
  /* 送る文の言い換え。数(何票か)は送らない=票数まで送ると
     ...
     **数を送らないのだから、数を名指しする言い方もしない**(この対話での修正・2026-08-28)。
     直す前の TOGETHER_PHRASE は「五つの見方がそろってこの向きを指しています」と
     書いていたが、上の `togetherOf` が true を返すのは**票数が3・4・5のどれか**で
     あって5票とは限らない=3票・4票の日にも「五つそろって」と伝えていた。
     公開ページで実際に「五つの見方すべてが同じ方向を向いていて」という文が出て、
     すぐ下の値の欄の「四つの見方がこの向きに寄っており」と食い違った
     (1990-05-15 で実測。間合いの寄りは算命学・九星気学・西洋占星術・宿曜の4票)。
     **中継役は渡された見立てのとおりに書いただけなので、直すのは渡す側である。**
     票数そのものは送れない(上の粒度の縛り)ため、数を言わない言い方へ替える。 */
  var TOGETHER_PHRASE = '多くの見方が重なってこの向きを指しています';
  var SPLIT_PHRASE = '見方によって向きが分かれていて、この向きはそのうちの一部です';
  ...
  var WRITING_RULES = [
    ...
    '・見方がいくつそろっているかの数は渡していないので、見方の数を書かない。'
  ];
  ...
  /* ===== 受け取り側の門(D8・工程4) =====================================
     ...
     **画面へ出す直前にこちら側で当てる**(D8)。
     ...
     この下の `BANNED` と `JARGON_SCREEN` が**唯一の置き場所**である
     (cycle-0200・台帳 AI-L5)。テストの側(tests/banned-words.js)は
     ここから再輸出するだけで、表の実体を持たない——テストのファイルは
     公開されずブラウザからも読めないが、逆向き=テストがこのファイルを
     require する道は最初から通れたので、そちらへ寄せて二重管理を畳んだ
     (cycle-0124 は写しを持つ側で作り、AI4-1 が1本ずつ突き合わせていた)。
     **写しが戻ることは tests/ai-screen.spec.js の AI4-1 が塞ぐ**
     (参照の同一と、テスト側のソースに表の直書きが無いことの両方を見る)。 */
  ...
  /* 禁止表現の**かな書き**(工程6・cycle-0127)。
     ...
     **`JARGON_SCREEN`(画面に出さない語)のかな書きは、語を選んで止める**
     (cycle-0199・台帳 GATE-L1)。cycle-0127 は「読みがふつうの語と同じ形に
     なるものが多い」として一律に足さない側へ倒したが、読みは語ごとに違う=
     「ごぎょう」「じゅうにし」のように**かな書きしてもその語にしかならない読み**まで
     素通りさせる理由は無い。そこで下の `JARGON_KANA` に**重なりの無い読みだけ**を持ち、
     ふつうの語と同じ形になる読みは `JARGON_KANA_SKIP` に**読みと理由を1行ずつ**残す
     (置かない判断を人の記憶ではなく表に持たせる。13行すべてがどちらかに現れることは
     tests/ai-screen.spec.js の GATE7-1 が名前の突き合わせで見る)。
     止めすぎない側の縛り=重なる読みを含むふつうの文(しくみ・いくぶん・
     「〜なくていい」・いちばん・じっかん・きゅうそく・こうどう)が通ることは
     GATE7-3 が MUST_BLOCK と同じ重みで見る。 */
  var BANNED_KANA = [
    /* 断定・的中 */
    /かならず/, /カナラズ/, /ぜったい/, /ゼッタイ/,
  ...
  /* 画面に出さない語の**かな書き**(cycle-0199・台帳 GATE-L1)。
     name は上の `JARGON_SCREEN` の name をそのまま指す(GATE7-1 が突き合わせる)。
     縛りの向きが漢字と違う語がある=
     ・はしら は「走ら(せ・ない・ず・ぬ・れ)」と「は+しら(べ・け)」
       (「まずはしらべて」「場がしらけて」)のかな書きと重なるので、続く字で除く
     ・じゅうにし は「じゅうにしゅ(十二種)」と「〜(に)じゅうにしても」を除くため、
       続く「ゅ」「て」を除く */
  var JARGON_KANA = [
    { re: /あんじゅう|わぜん|どくがい|ごうじゅう/, name: '七科分宿の呼び名' },
    { re: /はしら(?![せなずぬれべけ])/, name: '柱' },
    { re: /けいとう/, name: '系統' },
    { re: /ごぎょう|ゴギョウ/, name: '五行' },
    { re: /じゅうにし(?![ゅて])|じゅうだいしゅせい/, name: '十干・十二支・十大主星' }
  ];

  /* かなの行を**置かない**読みと、その理由(1読み1行)。
     読みがふつうの語のかな書きと同じ形になり、止めると中継役のまともな文が
     理由なく捨てられるもの。あちらの目的は言葉づかいの揃え(#51)なので、
     取りこぼしても読み手が傷つく形にはならない(断定・恐怖の BANNED とは重みが違う) */
  var JARGON_KANA_SKIP = [
    { name: '七科分宿の呼び名', yomi: 'きゅうそく', reason: '休息のかな書きと同じ形になる' },
    { name: '七科分宿の呼び名', yomi: 'けいそう', reason: '軽装・係争のかな書きと同じ形になる' },
    { name: '七科分宿の呼び名', yomi: 'もうあく', reason: '「もう開く」「もうあくび」のかな書きと同じ形になる' },
    { name: '組(組み合わせ・組む を除く)', yomi: 'くみ', reason: 'しくみ・とりくみ・くみあわせ などふつうの語の中に必ず現れる' },
    { name: '区分', yomi: 'くぶん', reason: 'いくぶん(幾分)と「動くぶん・続くぶん」(〜く分)のかな書きと同じ形になる' },
    { name: '盤', yomi: 'ばん', reason: 'いちばん・こんばん などふつうの語と同じ形になる' },
    { name: '定位', yomi: 'ていい', reason: '「〜なくていい」「〜していい」と同じ形になる' },
    { name: '黄道', yomi: 'こうどう', reason: '行動のかな書きと同じ形になる' },
    { name: '十干・十二支・十大主星', yomi: 'じっかん', reason: '実感のかな書きと同じ形になる' },
    { name: 'エレメント', yomi: 'えれめんと', reason: '外来語のひらがな書きは実際の文に現れない' },
    { name: 'グループ', yomi: 'ぐるーぷ', reason: '外来語のひらがな書きは実際の文に現れない' },
    { name: 'しるし', yomi: 'のしるし', reason: '網そのものが元からかな書きで、かなの行を別に持つ意味が無い' }
  ];

  /* 見方の数を名乗る文を止める(この対話での修正・2026-08-28)。
     **こちらは票数を1つも送っていない**(上の TOGETHER_PHRASE の項)ので、
     中継役が「五つの見方すべてが」「見方のうち四つが」と書いたとしても、
     それは渡していない数を書いたものである=当たっていても偶然でしかない。
     しかも画面のすぐ下には計算で出した本当の数(「四つの見方がこの向きに寄っており」)が
     並ぶため、食い違えばそのまま読み手の目に入る(これが実際に起きた形)。
     依頼文の側でも「見方の数を書かない」と頼んでいるが、
     **書いたから守られるとは数えない**ので受け取り側でも当てる(D8)。

     **止めるのは「見方」に近いところの数だけ**=「上の二つの手がかり」のように
     こちらが渡した手がかりの数を書くのは書き方の決まりが求めている形なので通す。
     「一つの見方として」のようなふつうの言い回しも巻き込んで止まるが、
     止まった先は決定論的な総合占いの文(=数が正しい側)なので、
     取りこぼして食い違った数を見せるより安全な側へ倒した。

     この表は**画面の門だけの道具**で tests/banned-words.js へは再輸出しない
     ——あちらは BANNED / JARGON_SCREEN をエンジンの文章そのものへ当てる走査に使われ、
     エンジン側の決定論的な文は「四つの見方が」と**正しい数を書くのが仕事**だからである
     (ここへ混ぜると、正しい文が禁止語として赤になる)。 */
  var COUNT_CLAIM = [
    { re: /[0-9０-９一二三四五六七八九十]\s*[つ個]の?見方/, name: '見方の数(前に置く形)' },
    { re: /見方.{0,5}[0-9０-９一二三四五六七八九十]\s*[つ個]/, name: '見方の数(後に置く形)' },
    { re: /(すべて|全て|全部).{0,3}見方/, name: '見方がすべて(前に置く形)' },
    { re: /見方.{0,3}(すべて|全て|全部)/, name: '見方がすべて(後に置く形)' }
  ];
  /* 門の一覧ここまで */
  ...
  function screenProblems(text) {
    ...
    for (i = 0; i < JARGON_KANA.length; i++) {
      if (JARGON_KANA[i].re.test(body) || JARGON_KANA[i].re.test(flat)) {
        problems.push('画面に出さない語のかな書き(' + JARGON_KANA[i].name + ')');
      }
    }
    for (i = 0; i < COUNT_CLAIM.length; i++) {
      if (COUNT_CLAIM[i].re.test(body) || COUNT_CLAIM[i].re.test(flat)) {
        problems.push('渡していない数を名乗る文(' + COUNT_CLAIM[i].name + ')');
      }
    }
    return problems;
  }
  ...
  var UI_TEXT = {
    ...
    aiHeading: '今日の読み物',
    aiNotice: 'この読み物は画面を開き直すたびに書き起こされるため、同じ生年月日でも文章が変わることがあります。同じ画面で読み直したときは、同じ文章が出ます。下の一覧に出る占いの結果そのものは変わりません。',
  ...
  return {
    ...
    JARGON_KANA: JARGON_KANA,
    JARGON_KANA_SKIP: JARGON_KANA_SKIP,
